...
model {
    ...
    for (t in 1:T) {
        ...
        if (RiskType[t] == 1)
            evGamble = 0.9 * (pow(RewardType[t], RiskAversion)) + 0.1 * (pow(RewardType[t], RiskAversion) - log(PainAvoidance * n_shocks + 1));
        else if (RiskType[t] == 2)
            evGamble = 0.5 * (pow(RewardType[t], RiskAversion)) + 0.5 * (pow(RewardType[t], RiskAversion) - log(PainAvoidance * n_shocks + 1));
        else
            evGamble = 0.1 * (pow(RewardType[t], RiskAversion)) + 0.9 * (pow(RewardType[t], RiskAversion) - log(PainAvoidance * n_shocks + 1));
