...
parameters {
    ...
    real<lower=0, upper=10> PainAvoidance;
...
model {
    ...
    for (t in 1:T) {
        ...
        if (RiskType[t] == 1)
            evGamble = 0.9 * (pow(RewardType[t], RiskAversion)) + 0.1 * (pow(RewardType[t], RiskAversion) - log(PainAvoidance));
        else if (RiskType[t] == 2)
            evGamble = 0.5 * (pow(RewardType[t], RiskAversion)) + 0.5 * (pow(RewardType[t], RiskAversion) - log(PainAvoidance));
        else
            evGamble = 0.1 * (pow(RewardType[t], RiskAversion)) + 0.9 * (pow(RewardType[t], RiskAversion) - log(PainAvoidance));
