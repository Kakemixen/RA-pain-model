...
model {
    ...
    int n_shocks = 0; //remember to reset for each sequence of trial / subject
        ...
        for (t in 1:Tsubj[i]) {
            ...
            if (RiskType[i,t] == 1)
                evGamble = pow(RewardType[i,t], RiskAversion[i]) - 0.1 * ( PainAvoidance[i] );
            else if (RiskType[i,t] == 2)
                evGamble = pow(RewardType[i,t], RiskAversion[i]) - 0.5 * ( PainAvoidance[i] );
            else
                evGamble = pow(RewardType[i,t], RiskAversion[i]) - 0.9 * ( PainAvoidance[i] );
